...
6. **Users = the six-role union.** The Users admin page (`adminhtml/rolemanagement/index`) is the union of `admin_user` (Trainer / Developer / Marketing / Admin / Super Admin operators) and `customer_entity` (Learner). Every storefront-registered account is automatically a Learner; no row in `mmd_user_role_map` is required for them. Operators carry their Role Manager role(s) from `mmd_user_role_map`.
...
- Don't auto-create admin_user rows for storefront customers (mmd_user_role_map is for operators only; learners stay as customer_entity).
