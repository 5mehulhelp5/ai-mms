-- Add admin user Dr Albert Ndeze (info.tertiarycourses.ng@gmail.com) with all 6 roles
-- and Administrators ACL group. Password: Welcome2026! (sha256("NW"+password):NW,
-- same hashing convention as migration 017).
-- Idempotent: INSERT IGNORE / re-derived joins; safe to re-run.

INSERT IGNORE INTO admin_user
    (firstname, lastname, email, username, password, created, is_active)
VALUES
    ('Dr Albert', 'Ndeze', 'info.tertiarycourses.ng@gmail.com',
     'info.tertiarycourses.ng@gmail.com',
     '0df0c8c38051fd31b37ec459d68e502998b321f43d2fa8b956c0fffa36566717:NW',
     NOW(), 1);

-- Assign Administrators ACL group (role_id=1). Remove any pre-existing
-- user-role assignment first so re-runs converge to a single Administrators row.
DELETE ar FROM admin_role ar
INNER JOIN admin_user u ON u.user_id = ar.user_id
WHERE u.email = 'info.tertiarycourses.ng@gmail.com'
  AND ar.role_type = 'U';
...
-- Seed all 6 roles in mmd_user_role_map; admin = primary.
...
-- Ensure admin is marked primary even if the row pre-existed with is_primary=0.
